...
The right-click menu's **Sync System Clock to GPS** sets the Mac's system clock to match the connected GPS receiver's time (admin authentication required). It's only enabled while there's a fix received within the last 5 seconds — with no GPS attached, no device selected, or no lock yet, it's disabled outright, and a GPS that's lost satellite lock (antenna covered, indoors, etc.) without the serial port itself disconnecting can't leave it enabled against a frozen, increasingly stale reading either. Getting the sync itself accurate runs into one hard limit and one tunable one:
